...
export class OfertasService{
  ...
  addOferta(oferta : Oferta){
    ...
    getOferta(nombre : string){
	    return this.http.get('getOferta/'+nombre)
	      .map(response => response.json())
	      .catch(error => this.handleError(error));
  	}
  	getJuegosOferta(nombre : string){
  		return this.http.get('getJuegosOferta/'+nombre)
  			.map(response =>response.json())
  			.catch(error=>this.handleError(error));
  	}
  	
    deleteOferta(nombre :string){
    	let headers = new Headers({
	   'X-Requested-With': 'XMLHttpRequest'
	});
	let options = new RequestOptions({ headers });  
	  
    return this.http.delete('deleteOferta/'+ nombre, options)
  	...
  	updateOferta(oferta : Oferta){
   	...
   	return this.http.put('editarOferta/'+oferta.nombre,body,options)
 			.map(response=>response.json())
 			.catch(error=>this.handleError(error));
  
   }
   guardarJuegos(nombre : string, juegos :juegos[]){
   		let body=JSON.stringify(juegos);
   		let headers = new Headers({
   			'Content-Type':'application/json'
   		});
   		let options = new RequestOptions({ headers });
   		
   		return this.http.put('addJuegoOferta/'+nombre,body,options){
   			.map(response=>response.json())
   			.catch(error=>this.handleError(error));
   		}
   		}	
   	borrarJuego(nombre : string, nombreOferta: string){
   		...
   		return this.http.put('deleteJuegoOferta/'+nombreOferta,body,options){
   			.map(response=>response.json())
   			.catch(error=>this.handleError(error));
